...
def main():
    ...
    list_df = pd.DataFrame(columns=['タイトル', 'url', 'ID', 'Title', 'main key', 'Category']) # 列名はConcatの5列の名を事前に入力？
    ...
    for mainKey, categ_num, keyPhrase in list(zip(mainKey_list, categ_num_list, keyPhrase_list))[44:]: #[36:]: # [:2]
        ...
        merge = df1.merge(sql_df[on], on=on, how='left', indicator=True).\
            ...
            reset_index(drop=True)                # ↑ 左＝df1にのみ存在する（(df2との重複を除く）タイトルにQueryしたあと、merge列を削除
        ...
        title_en_list_sr = pd.Series(title_en_list, name='Title', index=None).\
            reset_index(drop=True)
        # merge_en.columns = 'Title'  # 列名をTitleにすることで、SQLのTitle列に追加される
        print(title_en_list_sr)
    ...
    global concat2
    # print(list_df)
    concat2 = pd.concat([list_df, sr_sku], axis=1) # ループ蓄積したDFと一列のSKU列を横結合
    print(concat2)
...
# DF化
    # 説明、状態の DF化
    descon_df = pd.DataFrame(arr_descon, columns=['ID', '説明', '状態']).iloc[:, 1:3] #).iloc[:, :2] # 0. 1行目のみ
    # 画像のDF化
    img_spread(arr_img)
    #  価格のDF化
    pr_df = pd.DataFrame(arr_pr, columns=['ID','価格2種'])
    # print(pr_df)
    pr_df[['現在価格', '即決価格']] = pr_df['価格2種'].str.split(
        ', ', expand=True)#.iloc[:, [2, 3]] # [o, 2, 3] の0を消してID列削除
    # pr_df2 = pd.concat([pr_df, pr_df.iloc[:, 1].str.split(
    #     ', ', expand=True)], axis=1).iloc[:, [0, 2, 3]]  # drop([1], axis=1)

    # pr_df2.column = ['現在価格', '即決価格']
    pr_df = pr_df.iloc[:, [2, 3]]
    # print(pr_df)  # 0, 2, 3 列目のみ表示

    pr_csv = 'C:/Users/kazuki_juno/Desktop/00.Myself/04.Buyer/1.利益計算/db_check_yahoo4.csv'
    pr_df.to_csv(pr_csv, header=False, index=False)
    # print(concat2)
    concat3 = pd.concat([concat2, descon_df, grped, pr_df], axis=1) # concat2 と ここもインデックス揃える
                         # 説明と状態、画像、価格
    print(concat3)
    ...
    現在価格 = concat3['現在価格'].str.replace(' ', '').str.replace(',', '').astype('int64')
    即決価格 = pd.to_numeric(concat3['即決価格'].str.replace(' ', '').str.replace(',', '')) #.astype('int64'))
    # ilocかループで一行ずつ計算させるか、列ごと計算させるか
    #     列ごと置換し、列ごと計算
...
def img_spread(arr):  # (img_list):
    ...
    grped = df.groupby(df['ID'], sort=False).apply(f).iloc[:, 1:8].reset_index(drop=True) # おまけに、列名まで生成されてる!　ここで順を崩さないように
    # return grped
...
def remove_space_htmlTag(s): # 説明、状態用
    p = re.compile(r"<[^>]*?>") # htmlTagを削除　 htmlTagを削除するためにパターンをコンパイル
    remove = p.sub("", s) # ここでパターンに一致したものは空白に
    remove = re.sub(r'\s+', ' ', remove).strip()  # 連続する空白を1つのスペースに置き換え、前後の空白を削除した新しい文字列を取得する。
    return remove.replace('[', '').replace(']', '') # [] を置換
...
def remove_space_htmlTag_pr(s): # 価格用
    ...
    remove = p.sub("", remove) #（税込 11,620 円）
    # space = re.sub(r'(円（税込\d+円）)', space)

    remove = remove.replace('[', '').replace(']', '').replace('円（税 0 円） ', '')

    p = re.compile(r'[ぁ-んァ-ン一-龥]') # ひらがな・カタカナ・漢字すべてを取得
    return p.sub("", remove) # それを””に置換。　それだと13,7001　と繋がっている「1%」の1を置換できない
    # xx,xxx かx,xxx の数値のみに抽出、というのもできるはず

    # 「円（税込 XXX 円）」を置換する
    # 円ではじまり)で終わるものを中身含めてすべて対象にするには、正規表現
    # 円の手前の置換しない値段が４桁なら、のちの置換する（税込価格の）数値パターンは４桁、同じく５桁なら置換するパターンは５桁
